    ...
    */
         ...
         all_detections[i].begin(); it != all_detections[i].end(); ++it)
    {
      ...
      {//保存视频
        ...
        for (int j = 0; j < bboxes.size(); ++j)
        {
          cv::Point top_left_pt(cv::max(0.,bboxes[j].xmin()), cv::max(0.,bboxes[j].ymin()));
          cv::Point bottom_right_pt(cv::min(float(width-1),bboxes[j].xmax()), cv::min(float(height-1),bboxes[j].ymax()));
